"""
This is a forked version of python package "dirichlet" with some modifications and more functionality.
...
def _fit_s(D, a0, logp, tol=1e-7, maxiter=1000):
    ...
    for i in range(maxiter):
        ...
        if abs(s1 - s0) < tol:
            return a
    warnings.warn(f"Failed to converge after {maxiter} iterations, " f"s is {s1}")
    return a
...
def _fit_m(D, a0, logp, tol=1e-7, maxiter=1000):
    ...
    for i in range(maxiter):
        ...
        if norm(a1 - a0) < tol:
            return a1
        a0 = a1
    warnings.warn(f"Failed to converge after {maxiter} iterations, " f"s is {s}")
    return a1
