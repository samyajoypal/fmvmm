...
from fmvmm.mvem.stats.gig import _besselM3, _check_gig_pars
from fmvmm.mvem.stats import gig
import warnings
import math
...
def rvs(lmbda, chi, psi, mu, sigma, gamma, size=1):
    ...
    #return np.sqrt(W) * (Z @ A) + np.tile(mu, (size, 1)) + np.outer(W, gamma)
...
def fitghypmv(
    ...
    while (abs_closeness > abstol) and (rel_closeness > reltol) and (i < nit):
        ...
        if alpha_bar==0 and lmbda > 0 and not opt_pars["alpha_bar"] and opt_pars["lmbda"]:
            ...
            tmp_fit = scipy.optimize.minimize(t_optfunc, custom_log(-1 - lmbda), args=(delta_sum, xi_sum, n))
